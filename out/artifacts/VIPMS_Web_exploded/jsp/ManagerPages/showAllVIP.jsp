...
<div class="container">
    ...
    <div class="table-header">
        ...
        <!-- 添加冻结/解冻表单 -->
        <form id="batchFreezeForm" action="${pageContext.request.contextPath}/AllVIPServlet" method="post">
            <input type="hidden" name="type" value="freeze">
            <input type="hidden" name="vipid" id="batchFreezeVipId">
        </form>
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                <%--<div class="btn-group">
                    <a class="btn <%=v.isFreezed() ? "btn-unfreeze" : "btn-freeze"%>" id="freezeBtn"
                       href="${pageContext.request.contextPath}/AllVIPServlet?type=freeze&vipid=<%=v.getID()%>">
                        <%=v.isFreezed() ? "解冻" : "冻结"%>
                    </a>
                </div>--%>
                <div class="btn-group">
                    <%--<button class="btn <%=v.isFreezed() ? "btn-unfreeze" : "btn-freeze"%>" id="freezeBtn"
                            data-vipid="<%=v.getID()%>"
                            data-isfrozen="<%=v.isFreezed()%>">
                        <%=v.isFreezed() ? "解冻" : "冻结"%>
                    </button>--%>
                        <button class="btn <%=v.isFreezed() ? "btn-unfreeze" : "btn-freeze"%> freeze-action-btn"
                                data-vipid="<%=v.getID()%>"
                                data-isfrozen="<%=v.isFreezed()%>">
                            <%=v.isFreezed() ? "解冻" : "冻结"%>
                        </button>
                        ...
                        <%--<!-- 添加冻结/解冻表单 -->
                        <form id="freezeForm_<%=v.getID()%>" action="${pageContext.request.contextPath}/AllVIPServlet" method="post">
                            <input type="hidden" name="type" value="freeze">
                            <input type="hidden" name="vipid" value="<%=v.getID()%>">
                        </form>--%>
                </div>
            </td>
        </tr>
    ...
    $('.remark-cell').click(function() {
        ...
        inputField.keypress(function(e) {
            if (e.which === 13) { // Enter键
                $(this).blur();
                // 添加操作日志
                addLog('undoAddRemark', {
                    id: vipId,
                    historyremark: originalRemark,
                    newremark: nowRemark
                });
            }
        });
...
<!-- 冻结/解冻按钮点击事件 -->
<script>
    $(document).on('click', '.freeze-action-btn', function() {
        var button = $(this);
        var vipId = button.data('vipid'); // 正确获取数据
        //var isFrozen = button.hasClass('btn-unfreeze');
        var status = button.hasClass('btn-unfreeze') ? '冻结' : '解冻';
        var action = button.hasClass('btn-unfreeze') ? '解冻' : '冻结';

        if (confirm('确定要'+action+'会员'+vipId+'吗？')) {
            // 添加操作日志
            addLog('undoFreeze', {
                vipId: vipId,
                oldStatus: status,
            ...
            // 传递会员ID
            $('#batchFreezeVipId').val(vipId);
            // 提交表单
            $('#batchFreezeForm').submit();
        }
