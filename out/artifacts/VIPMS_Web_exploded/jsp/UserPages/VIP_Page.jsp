...
<div class="header">
    <h2>商城</h2>
    <div class="links">
        <%--会员中心——展示会员名、会员等级、余额、积分——余额充值——修改个人信息--%>
        <%--排行榜--%>
        <%--购买商品时减余额或积分。加对应的积分--%>
        <%
            VIP v = (VIP)request.getSession().getAttribute("vip");
        %>
        <%--<a href="${pageContext.request.contextPath}/jsp/UserPages/VipCenter?vipId=<%v%>.jsp">
            你好，<%= v.getRankMember()%>会员<%=v.getName()%></a>--%>
        <a href="${pageContext.request.contextPath}/jsp/UserPages/VipCenter.jsp"
           onclick="<% request.getSession().setAttribute("vip", v); %>">
            你好，<%= v.getRankMember()%>会员<%=v.getName()%></a>
        <a href="${pageContext.request.contextPath}/jsp/UserPages/ToBeVIP.jsp"
           onclick="<% request.getSession().setAttribute("vip", v); %>">VIP升级</a>
